...
## start when the cluster is busy.#SBATCH --mem=10G
...
#SBATCH --output=/project/jesu4262/1_linux/rnaseq/mapped_reads/%j_%x.out
#SBATCH --error=/project/jesu4262/1_linux/rnaseq/mapped_reads/%j_%x.err

## ################### CODE TO RUN ##########################
# Load modules (if required - e.g. when not using conda) 
# module load R-base/4.3.0

# Using samtools to generate a bam, sort and index it)
#samtools view (add an option to include header)
#samtools sort
#samtoos index
# -@ specifies the humber of threads

samtools view -@ 12 -b -h /project/jesu4262/1_linux/rnaseq/mapped_reads/sample1.sam > /project/jesu4262/1_linux/rnaseq/mapped_reads/ERR17550082.bam
samtools sort -@ 12  /project/jesu4262/1_linux/rnaseq/mapped_reads/ERR17550082.bam > /project/jesu4262/1_linux/rnaseq/mapped_reads/ERR17550082.sorted.bam
samtools index -@ 12  /project/jesu4262/1_linux/rnaseq/mapped_reads/ERR17550082.sorted.bam
